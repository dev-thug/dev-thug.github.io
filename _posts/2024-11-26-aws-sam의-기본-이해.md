...
> 
> **Reference:** [[AWS SAM에서 Lambda 함수의 Type과 API Gateway의 설정|AWS SAM에서 Lambda 함수의 Type과 API Gateway의 설정]]
...
**5.5 로컬에서 함수 실행 및 테스트**

로컬에서 Lambda 함수를 실행하려면 Docker가 실행 중이어야 합니다.
```bash
sam local invoke HelloWorldFunction --event events/event.json
```
• **–event:** 테스트 이벤트를 지정합니다.

API Gateway를 로컬에서 에뮬레이션하려면:
```bash
sam local start-api
```
브라우저나 API 클라이언트에서 http://localhost:3000/hello에 접속하여 응답을 확인합니다.
